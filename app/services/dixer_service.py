...
def generate_dixer(
  ...
  """
  Generate a complete Dixer (question and answer) using AI.
  ...
  Returns:
    Dict[str, str]: Dictionary with 'question', 'answer', and 'raw' keys.

  Raises:
    Exception: If the API call fails for any reason.

  For Python beginners:
  - Raises: tells you what errors this function might throw
  - You should wrap calls to this function in a try/except block
  """
  # Get the configured AI client
  client = get_openai_client()

  # Build the prompt from the user's inputs
  ...
  # Make the API call to generate the dixer
  # Note: This is a blocking call - it waits for the full response
  response = client.chat.completions.create(
    model="anthropic/claude-sonnet-4",  # Which AI model to use
    messages=[
      {"role": "system", "content": DIXER_SYSTEM_PROMPT},
      {"role": "user", "content": user_prompt}
    ],
    temperature=0.7,  # How creative the AI should be (0.0-1.0)
  ...
  return parse_dixer_response(ai_response)
